...
export default function EditShiftModal({
  ...
  // Populate form when shift data changes
  useEffect(() => {
    if (isOpen && shift) {
      // Parse the start time
      const startDate = new Date(shift.startTime);
      const startHour12 = getHour12(startDate);
      const startMinute = getMinute(startDate);
      const startPeriod = getTimePeriod(startDate);

      // Parse the end time
      const endDate = new Date(shift.endTime);
      const endHour12 = getHour12(endDate);
      const endMinute = getMinute(endDate);
      const endPeriod = getTimePeriod(endDate);
      ...
      setFormData({
        ...
        chargedToClient: shift.chargedToClient || false,
        specialShift: shift.specialShift || false,
        publishAndNotify: false,
        task: shift.task || "",
        jobRefNo: shift.jobRefNo || "",
        notes: shift.notes || "",
        notesToEmployee: shift.notesToEmployee || "",
        certLicense: shift.certLicense || "",
      });
    }
  }, [isOpen, shift]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[hsl(var(--color-card))] rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          ...
          {/* Footer */}
          <div className="flex items-center justify-between gap-3 p-4 border-t border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface-elevated))]">
            <Button
              type="button"
              variant="outline"
